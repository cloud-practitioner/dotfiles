...
- herdr, Claude Code, and Pi on Linux are what each vendor's own installer installs, pinned in `tools/` (never nixpkgs builds), so the WSL2 workstation and devcontainers can share versions. Change them only with `nix run .#update-tools` and keep their self-updaters disabled; README "Upstream CLI tools" has the details, including the devcontainer image's own copies.
